...
<h1 class="page-header">
    Tambah Bentuk Evaluasi
</h1>

<x-alert type="error" :message="session('error')" />

<form action="{{ route('rps.bentuk-evaluasi.store', $rps->id) }}"
      method="POST"
      class="card">
    ...
    <div class="form-group">
        ...
        <select name="sub_cpmk"
            ...
            @foreach($subCpmks as $subCpmk)
                <option value="{{ $subCpmk }}"
                        {{ old('sub_cpmk') === $subCpmk ? 'selected' : '' }}>
                    {{ $subCpmk }}
                </option>
            @endforeach
        </select>
    ...
    <div class="form-group">
        ...
        <textarea name="instrumen"
                  ...
                  rows="3"
                  placeholder="Contoh: Umpan balik tugas, Rubrik penilaian tugas kelompok">{{ old('instrumen') }}</textarea>
    ...
    <div class="form-group">
        ...
        <textarea name="tagihan"
                  class="form-textarea w-full"
                  rows="3"
                  placeholder="Contoh: Lembar jawaban UTS, dokumen PMP, screenshot Jira/Trello">{{ old('tagihan') }}</textarea>
