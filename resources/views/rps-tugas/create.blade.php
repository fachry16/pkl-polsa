...
<form action="{{ route('rps.tugas.store', $rps->id) }}"
    ...
    <div class="form-group">
        ...
        <select name="kategori_komponen" class="form-select w-full" required style="width: 100%; padding: 0.5rem; border-radius: 6px; border: 1px solid #cbd5e1;">
            <option value="tugas" {{ old('kategori_komponen', 'tugas') === 'tugas' ? 'selected' : '' }}>Tugas (TTi / TTk)</option>
            <option value="quiz" {{ old('kategori_komponen') === 'quiz' ? 'selected' : '' }}>Kuis / Quiz</option>
            <option value="uts" {{ old('kategori_komponen') === 'uts' ? 'selected' : '' }}>UTS (Ujian Tengah Semester)</option>
            <option value="uas" {{ old('kategori_komponen') === 'uas' ? 'selected' : '' }}>UAS (Ujian Akhir Semester)</option>
            <option value="praktikum" {{ old('kategori_komponen') === 'praktikum' ? 'selected' : '' }}>Praktikum / Responsif</option>
            <option value="project" {{ old('kategori_komponen') === 'project' ? 'selected' : '' }}>Project (Tugas Akhir / Project Base)</option>
        </select>
        ...
        <select name="sub_cpmk"
            ...
            @foreach($cpmks as $cpmk)
                <option value="{{ $cpmk->kode_cpmk }}"
                        {{ old('sub_cpmk') === $cpmk->kode_cpmk ? 'selected' : '' }}>
    ...
    <div class="form-group">
        ...
        <textarea name="deskripsi"
                  ...
                  rows="4"
                  placeholder="Isi deskripsi rincian instruksi tugas di sini...">{{ old('deskripsi') }}</textarea>
