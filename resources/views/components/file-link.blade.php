@props(['file', 'compact' => false, 'href' => null, 'nama' => null])

@php
    $nama = $nama ?? basename($file);
...
@if($file)
    <a href="{{ $url }}" target="_blank" rel="noopener"
       ...
       style="padding: 0.25rem 0.6rem; font-size: 0.75rem; {{ $compact ? 'padding: 0.2rem 0.5rem; font-size: 0.7rem;' : '' }}">
        ...
        {{ $nama }}
        @if($ekstensi)
            <span style="background: #FFF3C4; color: #A16207; border-radius: 4px; padding: 0.05rem 0.35rem; font-size: 0.6rem; font-weight: 700; margin-left: 0.35rem; text-transform: uppercase;">{{ $ekstensi }}</span>
        @endif
    </a>
@endif
