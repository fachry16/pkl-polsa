...
<div class="card marquee-header" style="padding: 0.5rem 0.85rem; margin-bottom: 1.25rem; display: flex; align-items: center; gap: 0.75rem; background: #ffffff; border: 1px solid #e2e8f0; border-radius: 10px; overflow: hidden;">
    {{-- Fixed Badge on Left --}}
    <div class="marquee-badge" style="display: flex; align-items: center; gap: 0.35rem; background: #FFF3C4; border: 1px solid #FFF3C4; color: #A16207; font-size: 0.72rem; font-weight: 700; padding: 0.25rem 0.6rem; border-radius: 6px; white-space: nowrap; flex-shrink: 0; z-index: 2;">
...
@else
    ...
    @php
        $availableTabs = [];
        $lockedDirektur = auth()->user()->isDosen() && auth()->user()->isDirektur() && ! session('sidebar_show_direktur', false);
        $lockedKaprodi = auth()->user()->isDosen() && auth()->user()->isKaprodi() && ! session('sidebar_show_kaprodi', false);
        ...
        if(auth()->user()->isKaprodi() && ! $lockedKaprodi) {
        ...
        if(auth()->user()->isDirektur() && ! $lockedDirektur) {
